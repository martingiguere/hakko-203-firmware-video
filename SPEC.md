...
6. **TODO**: Manual review of F5217-F5221 — scrollbar says `$0E530` but F5219 was misassigned to `$00020-$00090` (avg address confidence 0.295). Neighboring frames F5217, F5218, F5220, F5221 not in frame_assignments (dup-skipped or no valid results). These frames are in the late ROM region and may contain recoverable data for the `$0E5xx` range.
7. **TODO**: Review 6 accepted addresses skipped by `build_review_training.py` — training samples not extracted because frames don't match frame_assignments or have no crop_index entry: `$002A0` (1 frame, no assignment match), `$003D0` (no crop_index), `$003E0` (no crop_index), `$02400` (35 frames, no assignment match), `$11B50` (no crop_index), `$13F30` (no crop_index). These addresses have accepted bytes but contribute zero training data.
8. **TODO**: Fix 8↔5 data byte confusion — Claude vision spot-check of $0F2B0 (2026-03-21) found bytes 0, 4, 8 read as `08` by kNN but clearly `05` in 3 independent crops. This is a **data byte** error (not address), so trajectory correction doesn't help. Not in the documented confusion pairs (8/6, C/D, 4/9).
   - **Root cause**: Class imbalance — `5` has only **12 training samples** (vs 556 for `8`). Reference region has zero `05` bytes, so Pass 1 produces no `5` samples. 91.7% of `5` samples have `8` as nearest non-self neighbor. kNN classifies `5` as `8` with high confidence (often 1.0).
   - **Scope**: 1,294 byte positions across ROM have 05/08 disagreement in raw crop readings. In 408 cases (31.5%) `08` wins the vote and may be wrong.
   - **Other low-sample classes**: `3` (83 samples) and `D` (90 samples) may have similar undiscovered confusions.
   - **Possible fixes**:
     - **(a) More training samples for `5`**: Manually identify frames with confirmed `05` bytes (e.g., $0F2B0 via vision), extract digit cells, retrain. Even 50-100 samples would help. Could also augment the 12 existing samples.
     - **(b) Class-balanced kNN voting**: Normalize vote weights by inverse class frequency in `classify_batch()`. A `5` vote would count ~46x more than an `8` vote (556/12). Simple code change, no retrain.
...
5. **FF-fill erased flash gaps**: Fill confirmed erased regions with `0xFF`. Confirmed erased regions: `$04000`–`$04980` (Block 1 head — `$04670`–`$04750` confirmed all-FF visually, `$04760`–`$04980` never shown in video because the oscillation zone jumps directly from `$04750` to `$04990`, inferred FF from context), `$107F0`–`$12FFF` (erased ROM — last real data at `$107E0` is `64 00 70 17 FF...`, all FF from byte 4 onward), and `$13000`–`$13FFF` (tail of Block 0, confirmed from video frames 18842–19229). The `$0D070`–`$0DF8F` gap and other scattered ROM gaps are **not** erased flash — they are video coverage gaps surrounded by non-FF code data
